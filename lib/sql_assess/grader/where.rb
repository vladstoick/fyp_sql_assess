module SqlAssess::Grader
  class Where < Base
    ...
    def get_leaves(where_clause)
      if where_clause.nil?
        nil
      elsif where_clause[:is_inner] == false
        where_clause
      else
        [
          get_leaves(where_clause[:left_clause]),
          get_leaves(where_clause[:right_clause])
        ].flatten
      end
    end

    def match_score(where_clause_1, where_clause_2)
      if where_clause_1 == where_clause_2
        2
      else
        0
      end
